...
diabetes$pred_prob <- predict(logit_model, type = "response")

# Convert to class prediction at threshold = 0.5
diabetes$pred_class <- ifelse(diabetes$pred_prob > 0.5, 1, 0)

# Confusion matrix
table(
  Actual = diabetes$diabetes_binary,
  Predicted = diabetes$pred_class
)
```
...
roc_obj <- roc(diabetes$diabetes_binary, diabetes$pred_prob)
auc(roc_obj)
...
train_index = createDataPartition(diabetes$diabetes_binary, p = 0.7, list = FALSE)
